...
def occumpy_mem(cuda_device, _id):
    total, used = check_mem(cuda_device)
    total = int(total)
    used = int(used)
    max_mem = int(total * 0.9)
    block_mem = max_mem - used
    cuda_device = torch.device("cuda:{}".format(_id))
...
def select_head(attention: torch.Tensor, id2token: Dict, tokens: List[Token], Spans) -> List[List[Token]]:
    ...
    # half_attention_num = attention_window_size + 1
        ...
        for h_idx in range(head_nums):
            ...
            while t_idx < t_len:
                ...
                if token_s in special_tokens:
                    ...
                    t_idx += 1
                    continue
                if token_s.replace('Ġ', '').lower() in stop_words and (t_idx == len(id2token) - 1 or check_prefix_space(id2token[t_idx + 1].s)):
                    spanized_seq.append([token])
                    t_idx += 1
                    continue
...
def collect_span_in_specific_head(paper_id, Final_raw_spans, trivial_span_ids, full_text_attention,
    ...
    for text_attention in full_text_attention:
        ...
        g.span_collector_via_dfs(threshold=0.7)
...
def main():
    input_file = '../data/NarrativeQA/narrativeqa-train.json'
    output_dir = './ulqa_data_v2_NarrativeQA'
    ...
    global stop_spans
    paper_id_to_paras, paper_ids = convert_qasper_context(input_file, stop_spans, existing_paper_ids)
    ...
    for paper_id, paras in tqdm(paper_id_to_paras.items(), total=len(paper_id_to_paras), desc='process papers...'):
        if paper_id == 'c5341b86ed3f2c1a146b0d7648ef7c2d':
            continue
        Final_raw_spans = [] # List[List[Node]]
        full_text_tokens = []
        ...
        # 这次就不进行filter了，因为用了benepar之后filter的情况会少很多
