...
class QasperBaseline(Model):
    def __init__(
        ...
        **kwargs
        ...
        self._rouge_l = Average()
        self._bleu_1 = Average()
        self._bleu_4 = Average()
        self._meteor = Average()
            ...
            if not self.training:
                ...
                for predicted_answer, gold_answer in zip(predicted_answers, gold_answers):
                    ...
                    for gold_answer_info in gold_answer:
                        ...
                        gold_answer_texts.append(gold_answer_info['text'])
                    ...
                    scores = self.nlgeval.compute_individual_metrics(gold_answer_texts, predicted_answer)
                    self._bleu_1(scores['Bleu_1'])
                    self._bleu_4(scores['Bleu_4'])
                    self._rouge_l(scores['ROUGE_L'])
                    self._meteor(scores['METEOR'])
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        rouge_l_score = self._rouge_l.get_metric(reset)
        bleu_1_score = self._bleu_1.get_metric(reset)
        bleu_4_score = self._bleu_4.get_metric(reset)
        meteor_score = self._meteor.get_metric(reset)
        return {
            "answer_f1": f1_score,
