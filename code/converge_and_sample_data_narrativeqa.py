...
SAMPLE_NUM = 10000
VERSION = 2
input_train_file = '../data/NarrativeQA/narrativeqa-train.json'
output_dir = sys.argv[1] # [./ulqa_data_v2_NarrativeQA, ./ulqa_data_v3_NarrativeQA]
output_file = os.path.join(output_dir, 'ulqa_narrativeqa_train.json')
output_sampled_file = os.path.join(output_dir, 'ulqa_narrativeqa_train_{}.json'.format(SAMPLE_NUM))
output_sampled_revised_file = os.path.join(output_dir, 'ulqa_narrativeqa_train_{}_revised.json'.format(SAMPLE_NUM))
qid_to_paper_id = {}
...
for key, paper in _qasper.items():
    ...
    if key not in generated_paper_id_to_qas:
        ...
        paper['qas'] = []
        continue
    # assert key in generated_paper_id_to_qas, '{}'.format(key)
